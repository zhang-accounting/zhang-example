...
INCOMES = [f"{COMPANY}:Salary", "Remuneration", "CitiBank:Interest", "Publisher:DogeRandomHouse", "Publisher:TheZhangBookGroup", "Publisher:Scgolastic"]
ASSETS = ["Paypal", "WeChat", "CitiBank:9999"]
LIABILITIES = ["CreditCard:Apple", "CreditCard:CitiBank:3423"]
EXPENSES = ["PersonalDiet:Lunch", "PersonalDiet:Dinner", "Clothing", "Medical", "Sport", "Transportaion", f"{COMPANY}:Tax", "RemunerationTax"]
...
EXPENSE_TYPE = [
    [ # lunch, dinner
        ...
        [
            [
                [*list(map(lambda item: f"Liabilities:{item}", LIABILITIES)), *list(map(lambda item: f"Assets:{item}", ASSETS))],
     ...
     [ # remuneration
        ...
        ["Remuneration of <How to learn Zhang>", "Annual Remuneration", "Remuneration of <Good Good study, Day Day Up>"],
        [
            [
                list(map(lambda item: f"Income:{item}", ["Publisher:DogeRandomHouse", "Publisher:TheZhangBookGroup", "Publisher:Scgolastic"])),
...
class Cli:
    ...
    def run(self):
        ...
        beginning_of_zhang = arrow.Arrow(target_month.year, target_month.month, 1, 1,1,1).shift(days=-1).format("YYYY-MM-DD HH:mm:ss")
        
        for account in ASSETS:
            amount = random.randint(130000, 170000)
            content.append(f"{beginning_of_zhang} balance Assets:{account} {amount} {COMMODITIES[0]} with pad Equity:{EQUITIES[0]}")
        ...
        for month_offset in [-3,-2,-1, 0]:
            ...
            time_of_salary = arrow.Arrow(target_month.year, target_month.month, 1, 8,0,0).format("YYYY-MM-DD HH:mm:ss")
            ...
            for day in range(1, calendar.monthrange(target_month.year, target_month.month)[1]):
                ...
                for [loop_executor, payee, narration, postings] in EXPENSE_TYPE:
                    for _loop_time in range(0, loop_executor()):
                        target_time = arrow.Arrow(target_month.year, target_month.month, day, random.randint(0, 23),random.randint(0, 59),random.randint(0, 59)).format("YYYY-MM-DD HH:mm:ss")
